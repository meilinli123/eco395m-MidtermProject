...
activity_colors = ['blue', 'green', 'red']
...
for i, activity in enumerate(activities_of_interest):
    ...
    ax1.bar(x_positions + i * 0.2, participation_rate, width=0.2, label=activity, color=activity_colors[i])
...
ax1.set_xticklabels(unique_states)


for i, state in enumerate(unique_states):
    ax1.text(i + 0.2, -10, state, ha='center')
...
ax2.set_xticklabels(unique_states)


for i, state in enumerate(unique_states):
    ax2.text(i, -10, state, ha='center')
